...
public class SanPhamDAO {
    ...
    public int getIdSPCT(SanPham sp) {
        int i = 0;
        try {
            String query = "SELECT ID FROM SPCT where  mau_sac_id = ? and hinh_dang_id = ? and san_pham_id = ? ";
            PreparedStatement ps = con.prepareStatement(query);
            ps.setInt(1, sp.getMauSacId());
            ps.setInt(2, sp.getHinhDangId());
            ps.setInt(3, getId(sp));
            ps.executeQuery();
            ResultSet rs = ps.getResultSet();
            while (rs.next()) {
                i = rs.getInt(1);
    ...
    public void suaCTSP(SanPham sp) {
        try {
            String query = "update spct set gia = ?, so_luong = ?, trang_thai = ?, mau_sac_id = ?, hinh_dang_id = ? where id = ?";
            PreparedStatement ps = con.prepareStatement(query);
